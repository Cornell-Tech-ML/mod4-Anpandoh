...
class CNNSentimentKim(minitorch.Module):
    ...
    3. Apply a Linear to size C (number of classes) Dropout with rate 25% #no relu
    ...
    def __init__(
        ...
        self.conv0 = Conv1d(embedding_size, feature_map_size, filter_sizes[0])
        self.conv1 = Conv1d(embedding_size, feature_map_size, filter_sizes[1])
        self.conv2 = Conv1d(embedding_size, feature_map_size, filter_sizes[2])
    ...
    def forward(self, embeddings):
        ...
        # Apply 1d convolutions w/ relu
        conv0_out = self.conv0(embeddings).relu()
        conv1_out = self.conv1(embeddings).relu()
        conv2_out = self.conv2(embeddings).relu()

        # Apply max-over-time pooling
        max0 = minitorch.max(conv0_out, 2)
        max1 = minitorch.max(conv1_out, 2)
        max2 = minitorch.max(conv2_out, 2)

        # Concatenate the max pooled features
        sum_out = max0 + max1 + max2
        sum_out = sum_out.view(batch, self.feature_map_size)

        #linear
        linear_out = self.linear(sum_out)
