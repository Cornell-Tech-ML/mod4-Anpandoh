...
class Mul(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Return the gradients, mulzipped"""
...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        # Compute the element-wise product of sigmoid_a with itself
        sigmoid_squared = a.f.mul_zip(sigmoid_a, sigmoid_a)
        # Compute the negative product of grad_output and sigmoid_squared
        neg_grad_output_sigmoid_squared = a.f.neg_map(
            a.f.mul_zip(grad_output, sigmoid_squared)
        )
        # Compute the final gradient by combining the positive and negative terms
        return a.f.add_zip(
            a.f.mul_zip(grad_output, sigmoid_a), neg_grad_output_sigmoid_squared
        )
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, dim: Tensor) -> Tensor:
        """Permute the tensor."""
        temp = [int(i) for i in dim._tensor._storage]
        ctx.save_for_backward(a, temp)
        # iterate through dim and put into tuple being passed to permute
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # iterate through dim and put into tuple being passed to permute
        dim_back = [dim.index(i) for i in range(len(dim))]
        grad_output = minitorch.Tensor(
            grad_output._tensor.permute(*dim_back), backend=a.backend
        )
...
class View(Function):
    ...
    def forward(ctx: Context, a: Tensor, shape: Tensor) -> Tensor:
        """View the tensor through given shape."""
...
def grad_central_difference(
    ...
    """Compute the central difference of a function at a point."""
    x = vals[arg]
    up = zeros(x.shape)
    up[ind] = epsilon
